...
use iced::{
    theme::{self, Theme},
    widget::{
     checkbox, container, scrollable,
      Column, Container, Row, Space, Text,
    },
    Background, Color, Element, Length,
};
use crate::{app::messages::Message, ui::{constant::{application_title_text, APPLICATION_VERSION}, func::gui::traits::create::CreateComponent, styles::{common::{HeaderStyle, SectionStyle, SidebarStyle}, component::StatusBadgeStyle, container::ContentContainerStyle}}};
use crate::GUIState;
...
impl MainView for GUIState {
                ...
                .push(
                    Container::new(
                        Column::new()
                            ...
                            .push(checkbox("Development", true, |b| Message::FilterMarket("development".into(), b)).size(14).text_size(14))
                            .push(checkbox("Design", true, |b| Message::FilterMarket("design".into(), b)).size(14).text_size(14))
                            .push(checkbox("Writing", true, |b| Message::FilterMarket("writing".into(), b)).size(14).text_size(14))
                            .push(checkbox("Marketing", true, |b| Message::FilterMarket("marketing".into(), b)).size(14).text_size(14))
                            .spacing(8)
                    )
                    .width(Length::FillPortion(1))
                    .style(theme::Container::Custom(Box::new(SectionStyle)))
    ...
    let search_bar = Container::new(
        Row::new()
            .push(self.create_button(&self.toolbar_buttons_state.connect.title, self.toolbar_buttons_state.connect.action.clone()))
            .push(self.create_button(&self.toolbar_buttons_state.discover.title, self.toolbar_buttons_state.discover.action.clone()))
            .push(self.create_button(&self.toolbar_buttons_state.publish.title, self.toolbar_buttons_state.publish.action.clone()))
            .push(self.create_button(&self.toolbar_buttons_state.view_proposals.title, self.toolbar_buttons_state.view_proposals.action.clone()))
            .push(self.create_button(&self.toolbar_buttons_state.view_task.title, self.toolbar_buttons_state.view_task.action.clone()))
            .spacing(15)
    )
    .style(theme::Container::Custom(Box::new(SectionStyle)))
    .width(Length::FillPortion(1))
    .padding(15);
        ...
        let task_list = Container::new(
            scrollable(
                Column::new()
                    ...
                    .push(self.create_task_item(
                        "Develop SNARK circuits for private token transfers. Must be compatible with existing ERC20 interfaces.",
                        "32.0 ETH",
                        "Circom, Noir",
                        "4h ago"
                    ))
                    .push(self.create_task_item(
                        "Fix state corruption bug in cross-chain bridge. Messages occasionally fail to process in correct order.",
                        "15.5 ETH",
                        "Rust, Solidity",
                        "6h ago"
                    ))
        ...
        let logs_panel = Container::new(
            scrollable(
                logs.into_iter().fold(Column::new().spacing(8), |column, log| {
                    column.push(
                        Row::new()
                            .push(
                                Text::new(format!(
