...
impl WorkView for GUIState {
    ...
    fn create_work_item_view<'a>(&self, work_broadcast: &'a WorkBroadcast) -> Element<'a, Message> {
        ...
        let contact_row: Row<'_, Message, Renderer> = Row::new()
            .push(Text::new(work.employer_address.as_deref().unwrap_or("Unknown")).size(14))
            .push(Text::new(peer_info.last_seen.to_string()).size(14));

        let top_row: Row<'_, Message, Renderer> = Row::new()
            .push(contact_row)
            .push(Space::with_width(Length::Fill))
            .push(
                Text::new("Expires in 3 days")
                    .size(12)
        ...
        Container::new(
            Column::new()
                .push(Row::new().push(top_row).spacing(10))
                .push(
                    Row::new()
                        .push(
                ...
                .push(
                    Row::new()
                        ...
                        .spacing(10)
                        .push(Space::new(
                            Length::Fixed(0.0.into()),
                            Length::Fixed(5.0.into()),
                        )),
                )
                .push(Space::new(Length::Fixed(0.0), Length::Fixed(5.0)))
                .push(
                    Button::new(Text::new("Send a proposal").size(16).style(Color::WHITE))
    ...
    where
        Self: Sized,
    {
        let instructions_text = Text::new(
            "Your on-chain credentials and wallet address will be used to submit this proposal.",
        )
        .size(14);
        ...
        let work_items = self
            .work_items()
            .iter()
            .fold(work_items_accum, |column, item| {
