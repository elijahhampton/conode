...
pub struct SyncState {
    ...
    chain_operator: Arc<tokio::sync::RwLock<ChainContext>>,
...
impl SyncState {
    ...
    pub async fn sync_to_latest_block(&mut self) -> Result<(), SyncError> {
        if !self
            ...
            .is_ok()
        {
            debug!("syncing process already active");
            return Ok(());
        }
    ...
    async fn attempt_sync(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        self.event_sender
            .send(SyncEvent::SyncStarted {
                ...
                to: latest_synced_block_num,
            })
            .map_err(|_| SyncError::Internal)?;

        // Sync in chunks
        for chunk in 0..chunks {
            let current = latest_synced_block_num + (chunk * self.chunk_size) + 1;
            let target = std::cmp::min(current + self.chunk_size - 1, latest_block_num);

            self.event_sender.send(SyncEvent::SyncProgress {
                current,
                target,
                percentage: ((chunk as f64 + 1.0) / chunks as f64 * 100.0),
            })?;
    ...
    async fn process_block_range(
        ...
        let chain_context = self.chain_operator.read().await;
